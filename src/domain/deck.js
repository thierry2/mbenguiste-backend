 ...
 * intacte) ; chaque carte reçoit `superLikedMe`. Tri stable, priorités :
 *   ① super-like reçu  ② boosté  ③ intention complémentaire  ④ ordre d'entrée.
 */
function orderDeck(cards, { superLikerIds = new Set(), boostedIds = new Set(), myIntention = null } = {}) {
  const marked = cards.map((c) => ({ ...c, superLikedMe: superLikerIds.has(c.id) }));
